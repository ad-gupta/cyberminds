...
const CreateJob = ({ open, setOpen, setReload }: {open: boolean, setOpen: any, setReload:any}) => {
  ...
  return (
    <>
      {open && (
        <div className="bg-white flex-col rounded-md px-8 h-full sm:h-[70%] max-md:mx-3 lg:w-[50%] md:w-[70%] absolute z-50 top-10 left-0 right-0 bottom-0 m-auto flex flex-col gap-8 pt-3 shadow-lg shadow-black">
          <p className="text-center font-bold text-lg">Create Job Opening</p>
          <p className="absolute top-3 right-8 text-lg cursor-pointer" onClick={() => setOpen(false)}>x</p>
              ...
              <div className="mb-4 w-full relative">
                ...
                <div className="relative">
                  <select
                    ...
                    className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none sm:text-sm appearance-none"
                  >
              ...
              <div className="mb-4 w-full">
                ...
                <div className="flex relative w-full gap-3">
                  <input
                    ...
                    id="salaryRange"
                    name="minSalary"
                    className="mt-1 pl-7 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                    value={minSalary}
                    onChange={(e) => setMinSalary(e.target.value)}
                    placeholder="₹0k"
                  />
                  <RiArrowUpDownFill className="absolute top-3 text-neutral-400 mx-2 m-1" />
                  <input
                    type="text"
                    id="salaryRange"
                    name="maxSalary"
                    className="mt-1 pl-9 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                    value={maxSalary}
                    onChange={(e) => setMaxSalary(e.target.value)}
                    placeholder="₹120k"
                  />
                  <RiArrowUpDownFill className="absolute top-3 max-sm:right-26 right-32 text-neutral-400 mx-2 m-1" />
                </div>
              </div>
              <div className="mb-4 w-full relative">
                <label
